...
namespace App_Service.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        public PaymentService(IVNPayService vnPayService, IOrderDetailService orderDetailService,
            IOrderService orderService, ICourseService courseService)
        {
        ...
        public async Task<PaymentLinkResponse> CreatePayment(Order order)
        {
            ...
            var orderInfo = await TakeOrderInfo(order.OrderId);
            ...
            VNPayRequest request = new VNPayRequest()
            {
                ...
                vnp_Amount = (int)Math.Ceiling(orderInfo.TotalPrice) * 100,
                vnp_CurrCode = vnPayConfig.CurrencyCode,
                vnp_OrderType = "other",
                vnp_OrderInfo = $"Ngày: {order.CreatedDate.ToString("yyyyMMddHHmmss")}; Số lượng: {orderInfo.ItemAmount}; Tổng giá: {orderInfo.TotalPrice}",
        ...
        public async Task<PaymentReturnResponse> CheckPaymentResponse(VNPayResponse response)
        {
            ...
            if (isValid)
            {
                if (await _orderService.GetOrderById(int.Parse(response.vnp_TxnRef)) != null)
                {
